...
class MenuBuilder
{
    ...
    {
        $tags = ['ul', 'ul-root', 'li', 'li-parent', 'a', 'a-parent', 'active-class'];
    ...
    public function setActiveItem($href, $activeClass = '')
    {
        ...
        $this->set('active-class', ['class' => $this->activeClass]);
    }
    ...
    public function insert($item, $before_at = '', $parent = '')
    {
        ...
        if ($parent === '')
        {
            ...
            if ($pos !== FALSE)
            {
                array_splice($this->arrData, $pos, 0, [$item]);
                return;
            }
            $this->arrData[] = $item;
        } else
        {
            ...
            if ($pos !== FALSE)
            {
                array_splice($this->arrData[$pos_parent]['children'], $pos, 0, [$item]);
                return;
            }
     ...
     */
    public function buildFromDb($result, $columnID, $columnParent)
    {
    ...
    {
        $items  = [];
        $result = DataType::buildObject($result);
        foreach ($result as $row)
        {
            $target = (isset($row->target)) ? $row->target : '_self';
            $icon = (isset($row->icon)) ? $row->icon : '';
            $items[$row->$columnParent][$row->$columnID] = [
                'id'          => $row->id,
                'href'        => $row->link,
                'text'        => $row->title,
                'icon_class'  => $icon,
     ...
     * @return string The string attributes
     */
