...
trait HumanizeTrait
{
    ...
    public static function bytesToReadable($size, $format = false, $longName = false) {
        $sizes = [
            'b'  => 'Bit',
            'B'  => 'Bytes',
            'KB' => 'Kilobytes',
        ...
        if($size <= $max_Yt){
            if ($size == 0){
                ...
                $formatted = round($size/pow(1024, ($i = floor(log($size, 1024)))), 2) .$space. ucfirst(strtolower($sizes[$i]));
            }
