...
class Country
{
    ...
    public function fetchCountryInfo($request)
    {
        $countries = self::getData();
        for ($i = 0; $i < count($countries); $i++){
            if (isset($countries[$i][strtolower(trim($request))])){
                return $countries[$i];
            }
        }
        return false;
    ...
    public function fetch($key1, $key2, $search)
    {
        $countries = self::getData();
        for ($i = 0; $i < count($countries); $i++){
            $value = $countries[$i];
            // in case the user accidentally or willingly used spaces, we will want to still validate that
            if ( (isset($value[$key1]) && strtolower($value[$key1]) == strtolower($search)) || (isset($value[$key1]) && strtolower($value[$key1]) == strtolower($search))) {
                return $value[$key2];
            }
        }
        return false;
    ...
    public function fetchByValue($value){
        foreach (self::getData() as $datum) {
    ...
    public function fetchByKeys($key1, $key2, $key3 = null, $format = "%s")
    {
        $temp = [];
        foreach (self::getData() as $value) {
    ...
    public function fetchName($countryCode)
    {
        return self::fetch('alpha2', 'name', $countryCode) ? self::fetch('alpha2', 'name', $countryCode) : self::fetch('alpha3', 'name', $countryCode);
    }

    public function fetchISOCode($value, $alpha2 = false){
        foreach (self::getData() as $datum) {
            if (isset($datum['name']) && trim($datum['name']) == trim($value)) {
                return strtoupper( trim( $alpha2 ? $datum['alpha2'] : $datum['alpha3'] ) );
            }
        }
        return false;
    }

    public function fetchCallingCode($countryCode, $prepend = false)
    {
        $alpha2 = self::fetch('alpha2', 'calling_code', $countryCode);
        $alpha3 = self::fetch('alpha3', 'calling_code', $countryCode);
        ...
        return ($prepend && $out) ? '+' . $out : $out;
    }

    public function fetchCurrency($countryCode)
    {
        $name = self::fetch('alpha2', 'currency_name', $countryCode) ? self::fetch('alpha2', 'currency_name', $countryCode) : self::fetch('alpha3', 'currency_name', $countryCode);
        $code = self::fetch('alpha2', 'calling_code', $countryCode) ? self::fetch('alpha2', 'calling_code', $countryCode) : self::fetch('alpha3', 'calling_code', $countryCode);
    ...
    public function fetchNameByCallingCode($code)
    {
        return self::fetch('calling_code', 'name', $code);
    }

    public function listAll($alpha2 = true){
        $countries = self::getData();
        $data = [];
        for ($i = 0; $i < count($countries); $i++){
            if ($alpha2){
